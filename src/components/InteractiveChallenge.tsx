...
export const InteractiveChallenge: React.FC<InteractiveChallengeProps> = ({ 
  ...
  // Analyze digits of the selected number
  const numString = String(currentNumber);
  const digits = numString.split('').map(Number);
  const correctAnswer = currentNumber * 11;

  // Derive shortcut values for display tips
        ...
        <div className="lg:col-span-5 flex flex-col gap-5">
              ...
              {PRESETS.map((preset) => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className="text-[10px] opacity-90 font-mono">{preset.number} × 11</span>
                  </button>
                );
              })}
            </div>
          ...
          <div className="bg-brand-charcoal border-4 border-slate-900 rounded-[32px] p-5 text-white shadow-xl">
            ...
            <p className="text-[11px] text-slate-300 leading-relaxed font-bold mb-4">
              选好上面的数字后，可以点击下方按钮，重新在故事动画中演示该数字的奇妙相加！
            </p>
          ...
          <div className="bg-white border-8 border-brand-peach rounded-[40px] p-6 shadow-2xl flex-grow flex flex-col justify-between min-h-[380px]">
            ...
            <div>
              <div className="flex justify-between items-start mb-4">
                ...
                <span className="text-[10px] text-brand-gray font-bold font-mono">乘11魔法小测试</span>
              </div>
 
              <div className="text-center my-4">
                <h4 className="text-xl sm:text-2xl font-black text-[#2D3436] tracking-wide font-display">
                  请问： <span className="text-brand-pink underline font-extrabold">{currentNumber}</span> × 11 = ？
                </h4>
              </div>
                  ...
                  <AnimatePresence mode="wait">
                    {showResult === true && (
                      ...
                      >
                        ...
                        <div className="text-left">
                          <span className="block text-brand-charcoal font-black text-sm mb-0.5">答对啦！🎉 {currentNumber} × 11 = <strong className="text-brand-pink font-display underline">{correctAnswer}</strong>！你太棒了！</span>
                          <span className="block text-[10px] text-brand-gray font-bold">首位是 {firstDigit}，末尾是 {lastDigit}，中间相加完美符合口诀！</span>
                        </div>
                      </motion.div>
                    )}
                    ...
                    {showResult === false && (
                      <motion.div
                        ...
                        animate={{ opacity: 1, scale: 1 }}
                        exit={{ opacity: 0 }}
                        className="bg-rose-50 border-3 border-brand-peach rounded-2xl p-3 text-rose-800 font-bold text-center text-xs flex flex-col items-center justify-center gap-0.5 shadow-md"
                      >
                        <span className="font-bold flex items-center gap-1 text-rose-700">
                          <span>🧙‍♂️ 哎呀，数字小火车说答案好像不完全对哦！再算一算：</span>
                        </span>
                        <span className="text-[10px] text-slate-500 mt-1">
                          试试提示：两头拉开是 {firstDigit} 和 {lastDigit}。中间加一加。
                        </span>
                      </motion.div>
                    )}
